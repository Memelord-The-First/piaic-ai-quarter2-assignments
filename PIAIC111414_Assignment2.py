...
def function10():
    ...
    brr = np.array([1,2,3])
    subt = np.where(arr == 3, arr[0] - brr[0], (np.where(arr == 4, arr[1] - brr[1], (np.where(arr == 5, arr[2] - brr[2], arr))))) # write your code here
...
def function12():
    ...
    arr = np.array([1,2,3])
    ans = np.hstack(np.append([np.append([np.repeat(1, 3), np.repeat(2, 3)], np.repeat(3, 3)), arr, arr], arr)) #write your code here
...
def function13():
    ...
    ans = arr[np.where(np.logical_and(arr > 5, arr < 10))] #write your code here
...
def function19():
    ...
    return (X*W)+b  # Write your code here
